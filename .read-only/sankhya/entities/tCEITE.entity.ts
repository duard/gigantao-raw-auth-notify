...
@Index('PK_TCEITE', ['nuimp', 'sequencia'], { unique: true })
@Entity('TCEITE', { schema: 'SANKHYA' })
export class TceiteEntity {
  ...
  @ManyToOne(() => TgfparEntity, (tgfparEntity) => tgfparEntity.tceites)
  @JoinColumn([{ name: 'CODPARCFAB', referencedColumnName: 'codparc' }])
  codparcfab: TgfparEntity;

  @ManyToOne(() => TsimoeEntity, (tsimoeEntity) => tsimoeEntity.tceites)
  @JoinColumn([{ name: 'CODMOEDA', referencedColumnName: 'codmoeda' }])
  codmoeda: TsimoeEntity;

  @ManyToOne(() => TgfproEntity, (tgfproEntity) => tgfproEntity.tceites)
  @JoinColumn([{ name: 'CODPROD', referencedColumnName: 'codprod' }])
  codprod: TgfproEntity;

  @ManyToOne(() => TcecabEntity, (tcecabEntity) => tcecabEntity.tceites, {
    onDelete: 'CASCADE',
  })
  @JoinColumn([{ name: 'NUIMP', referencedColumnName: 'nuimp' }])
  nuimp2: TcecabEntity;

  @ManyToOne(() => TgfempEntity, (tgfempEntity) => tgfempEntity.tceites)
  @JoinColumn([{ name: 'CODEMP', referencedColumnName: 'codemp' }])
  codemp: TgfempEntity;

  @ManyToOne(() => TgflocEntity, (tgflocEntity) => tgflocEntity.tceites)
  @JoinColumn([{ name: 'CODLOCAL', referencedColumnName: 'codlocal' }])
  codlocal: TgflocEntity;

  @ManyToOne(() => TgfvolEntity, (tgfvolEntity) => tgfvolEntity.tceites)
  @JoinColumn([{ name: 'CODVOL', referencedColumnName: 'codvol' }])
  codvol: TgfvolEntity;

  @OneToMany(() => TceratEntity, (tceratEntity) => tceratEntity.tceite)
