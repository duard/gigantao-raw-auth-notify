import { Column, Entity, Index } from 'typeorm';

@Index(
  'PK_TGFEFDC1020',
  ['codemp', 'dtref', 'regniv1', 'nuprocesso', 'seqprocesso'],
  {
    unique: true,
  },
)
@Entity('TGFEFDC1020', { schema: 'SANKHYA' })
export class Tgfefdc1020Entity {
  @Column('int', { primary: true, name: 'CODEMP' })
  codemp: number;

  @Column('datetime', { primary: true, name: 'DTREF' })
  dtref: Date;

  @Column('varchar', {
    primary: true,
    name: 'REGNIV1',
    length: 4,
    default: () => "'1001'",
  })
  regniv1: string;

  @Column('int', { primary: true, name: 'NUPROCESSO' })
  nuprocesso: number;

  @Column('smallint', { primary: true, name: 'SEQPROCESSO' })
  seqprocesso: number;

  @Column('varchar', { name: 'REGISTRO', length: 4, default: () => "'1020'" })
  registro: string;

  @Column('varchar', { name: 'NUM_PROC', nullable: true, length: 21 })
  numProc: string | null;

  @Column('varchar', { name: 'IND_NAT_ACAO', nullable: true, length: 2 })
  indNatAcao: string | null;

  @Column('datetime', { name: 'DT_DEC_ADM', nullable: true })
  dtDecAdm: Date | null;

  @Column('char', { name: 'DIGITADO', length: 1, default: () => "'N'" })
  digitado: string;
}
