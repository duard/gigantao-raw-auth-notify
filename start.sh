...
# This command uses awk/grep for parsing as yq might not be installed
docker compose --profile "$ENVIRONMENT" --env-file "$ENV_FILE" config | awk "/^services:/,/^networks:|^volumes:|^x-/" | awk "/^  $SERVICE_CONFIG_NAME:/,/^  [^[:space:]]/" | grep 'environment:' -A 10
echo "------------------------------------------------------"
...
if [ "$ENVIRONMENT" == "development" ] || [ "$ENVIRONMENT" == "local" ]; then
    docker compose --profile "$ENVIRONMENT" --env-file "$ENV_FILE" up --build --remove-orphans -e START_CMD="$APP_START_CMD" $SERVICES_TO_START
else
    docker compose --profile "$ENVIRONMENT" --env-file "$ENV_FILE" up --remove-orphans -e START_CMD="$APP_START_CMD" $SERVICES_TO_START
fi
